...
class Player(Bot):
    ...
    def assign_holes(self, hole_cards):
        ...
        holes_and_strengths = sorted(holes_and_strengths, key=lambda x: x[1], reverse = True) #sort them by strength
        ...
        for i in range(NUM_BOARDS):
            if AssignAction in legal_actions[i]:
                ...
                if street < 3: #pre-flop
                    if i == 0:
                        raise_ammount = int(my_pips[i] + board_cont_cost + 0.5 * (pot_total + board_cont_cost))
                    elif i == 1: 
                        raise_ammount = int(my_pips[i] + board_cont_cost + 0.2 * (pot_total + board_cont_cost))
                    else: 
                        raise_ammount = int(my_pips[i] + board_cont_cost + 0.1 * (pot_total + board_cont_cost))
                ...
                ## FOLD if it's most expensive board (board 3)
                if i == 2 and FoldAction() in legal_actions[i]:
                    my_actions[i] = FoldAction()
